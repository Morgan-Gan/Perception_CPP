...
void UpsampleLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  mask_data.Reshape(bottom[0]->num(), bottom[0]->channels(), upsample_h_, upsample_w_);
  Dtype* mask_data_ = mask_data.mutable_cpu_data();
  float scale_h = upsample_h_ / (float)height_;
  float scale_w = upsample_w_ / (float)width_;
...
void UpsampleLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const Dtype* bottom_mask_data = mask_data.cpu_data();
  ...
  for (int n = 0; n < bottom[0]->num(); ++n) {
    for (int c = 0; c < channels_; ++c) {
	  ...
	  bottom_mask_data += mask_data.offset(0, 1);
