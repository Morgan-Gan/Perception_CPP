...
#define CV_CAP_PROP_FRAME_COUNT (7)                                             // frames of video
...
float DRAW_CLASS_THRESH=0.2;        //// 0.6;
...
void cameraRead(int index) 
{
	int i = 0;
  	...
  	if (pthread_setaffinity_np(pthread_self(), sizeof(mask), &mask) < 0)                      // cpu mask set which cpu core to be uesed by processor
    		cerr << "set thread affinity failed" << endl;
