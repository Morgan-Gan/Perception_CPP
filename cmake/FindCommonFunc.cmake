...
function(link_static_share_lib pkg_lst pkgcfg_path pkg_type)
    #重新设置pkgcfg查找路径
    ...
    foreach(pkg ${pkg_lst})
        ...
        ELSEIF(pkg_type MATCHES "share")
            #根据配置信息，进行动态库关联
            target_link_libraries(${PROJECT_NAME} ${${uppername}_LDFLAGS})
        ENDIF()
    endforeach(pkg)
endfunction(link_static_share_lib)

# 追加依赖的公共对象
function(add_cmm_obj obj_name)
    foreach(sub_name ${obj_name})
        string(TOUPPER ${sub_name} uppername)
        SET(CMMOBJ_${uppername}_DIR ${CMMOBJ_DIR}/${sub_name})
